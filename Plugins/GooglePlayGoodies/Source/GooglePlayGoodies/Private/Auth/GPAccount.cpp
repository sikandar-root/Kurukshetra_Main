...
void UGPAccount::Init(jobject AccountObject)
{
	...
	GetServerAuthCodeMethod = FJavaWrapper::FindMethod(Env, AccountClass, "getServerAuthCode", "()Ljava/lang/String;", false);
}
#endif
...
FString UGPAccount::ServerAuthCode() const
{
	FString Result;
#if PLATFORM_ANDROID
	JNIEnv* Env = FAndroidApplication::GetJavaEnv();
	jstring IdString = static_cast<jstring>(Env->CallObjectMethod(Account, GetServerAuthCodeMethod));
	Result = FJavaHelper::FStringFromLocalRef(Env, IdString);
#endif
	return Result;
}
