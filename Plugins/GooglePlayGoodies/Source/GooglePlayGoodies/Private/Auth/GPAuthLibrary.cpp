...
#if PLATFORM_ANDROID
	UGooglePlayGoodiesSettings* Settings = FGooglePlayGoodiesModule::Get().GetSettings();
	auto ClientID = FJavaClassObject::GetJString(Settings->ClientID);
	GPMethodCallUtils::CallStaticVoidMethod(GPAuthClassName, "login", "(Landroid/app/Activity;ZZZZZZZLjava/lang/String;)V",
		FJavaWrapper::GameActivityThis, bDoSilentLogin,
		PermissionFlags.bId, PermissionFlags.bProfile, PermissionFlags.bEmail, PermissionFlags.bIdToken, PermissionFlags.bCloudSave, PermissionFlags.bServerAuthCode, *ClientID);
#endif
}
